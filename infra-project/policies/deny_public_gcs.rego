package main

deny[msg] {
  input.resource_changes[i].type == "google_storage_bucket"
  not input.resource_changes[i].change.after.uniform_bucket_level_access
  msg := "GCS bucket must have uniform bucket-level access enabled"
}

deny[msg] {
  input.resource_changes[i].type == "google_storage_bucket"
  not input.resource_changes[i].change.after.iam_configuration.public_access_prevention
  msg := "Public access prevention must be explicitly enabled"
}